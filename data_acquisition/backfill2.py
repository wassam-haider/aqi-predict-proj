...
# ---------------- MAIN BACKFILL ---------------- #
def backfill_specific_range():
    # Set exact dates
    end = datetime(2025, 12, 18, 16, 0, 0, tzinfo=timezone.utc)
    start = end - timedelta(days=60)  # 60 days before

    start_unix = int(start.timestamp())
    end_unix = int(end.timestamp())

    print(f"Fetching pollution history from {start.date()} to {end.date()}...")
    ...
    existing_timestamps = set(r["timestamp"] for r in collection.find({}, {"timestamp": 1, "_id": 0}))
    ...
    if records:
        ...
        print(f"\nInserted {len(records)} new records into MongoDB.")
    else:
        print("No new records to insert.")

if __name__ == "__main__":
    backfill_specific_range()